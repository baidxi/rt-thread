...
static void sunxi_gpio_pin_mode(struct rt_device *device, rt_base_t pin, rt_uint8_t mode)
{
...
static void sunxi_gpio_pin_write(struct rt_device *device, rt_base_t pin, rt_uint8_t value)
{
    struct sunxi_gpio *gpio = device->user_data;
    rt_uint32_t val;
    rt_uint32_t bank = pin / 32;
...
static rt_err_t sunxi_gpio_pin_irq_enable(struct rt_device *device, rt_base_t pin, rt_uint8_t enabled)
{
    return RT_EOK;
}
